...
class CrearReporteTS:
    def crearReporteErrores(self):
        ...
        '''
        for itemError in listaTablaSimbolos:
            contador = contador + 1
            dictTemp["No"] = contador
            dictTemp["Identificador"] = itemError.get("Identificador")
            dictTemp["Tipo"] = itemError.get("Tipo")
            dictTemp["Tipo2"] = itemError.get("Tipo2")
            dictTemp["Entorno"] = itemError.get("Entorno")
            dictTemp["Valor"] = itemError.get("Valor")
        ...
        path1 = os.path.join(my_path1, f"../ReporteSimbolos.html")
        path1 = path1.replace("\\", "/")
        path1 = path1.replace("../", "")
        ...
        path = os.path.join(my_path, f"../ReporteSimbolos.html")
        path = path.replace("\\", "/")
        path = path.replace("../", "")
